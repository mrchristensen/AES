...
class TestInvCipher(TestAES):
    ...
    def test_inv_mix_columns(self):
        np.set_printoptions(formatter={'int': hex})
        state = parse_input_string("bd6e7c3df2b5779e0b61216e8b10b689").reshape((4, 4)).T

        mix_expected_result = parse_input_string("4773b91ff72f354361cb018ea1e6cf2c").reshape((4, 4)).T
...
class TestCipher(TestAES):
    ...
    def test_cipher(self):
        ...
        out_expected_result = np.array([[0x39, 0x25, 0x84, 0x1d],
                                        [0x02, 0xdc, 0x09, 0xfb],
                                        [0xdc, 0x11, 0x85, 0x97],
                                        [0x19, 0x6a, 0x0b, 0x32]]).T

        # formatted_expected_out = create_byte_matrix(out_expected_result)
        #
        # print("formatted_expected_out:\n", formatted_expected_out)

        out_result = cipher(state, key, 128)
